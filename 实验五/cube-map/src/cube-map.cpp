...
// camera
Camera camera(vec3(0.0f, 1.5f, 6.0f));
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Cube Map", NULL, NULL);
	if (window == NULL) {
	...
	unsigned int boxMapTexture = loadTexture("res/textures/texture.jpeg");
	// skybox VAO
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		mat4 view = camera.GetViewMatrix();  // π€≤Ïæÿ’Û
		mat4 projection = mat4(1.0f);  // Õ∂”∞æÿ’Û
		model = rotate(model, (float)glfwGetTime() * radians(45.0f), vec3(0.5f, 1.0f, 0.0f));
		...
		glBindTexture(GL_TEXTURE_CUBE_MAP, boxMapTexture);
